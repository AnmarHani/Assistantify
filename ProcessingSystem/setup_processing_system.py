...
from AnalyticsSystem.prompt_functions import finance_prompt, health_prompt, productivity_prompt
...
async def send_to_gpt(message: str, user = None):
    content = "You are a helpful Assistant that helps in different domains, you can call yourself Assistantify. Give Helpful Insights and Suggestions."
    if "financ" in message.lower() or "مالي" in message.lower():
        content = finance_prompt(user)
    
    if "health" in message.lower() or "صحة" in message.lower()  or "صحي" in message.lower():
        content = health_prompt(user)
    
    if "productivity" in message.lower() or "إنتاج" in message.lower():
        content = productivity_prompt(user)
    
    if "clean" in message.lower() or "تنظيف" in message.lower() or "نظف" in message.lower()  or "نظافة" in message.lower():
        async with httpx.AsyncClient() as client:
            await client.get(f"http://{HOST}:{PORT}/device_on")
        return "Sure! I will start cleaning now."
        
    if "back" in message.lower() or "رجوع" in message.lower() or "إرجعي" in message.lower() or "رجع" in message.lower():
        async with httpx.AsyncClient() as client:
            await client.get(f"http://{HOST}:{PORT}/device_off")
        return "Okay, Turning Off."
    
    if "steps" in message.lower() or "achieve" in message.lower() or "apple" in message.lower() or "خطوات" in message.lower() or "خطوة" in message.lower() or "تفاح" in message.lower() or "إنجاز" in message.lower():
        async with httpx.AsyncClient() as client:
            await client.post(
        ...
        if "5,000" in message.lower() or "خمسة" in message.lower() or "خمس" in message.lower():
            return "Wow! You have Walked 5000 Steps, Here is 1 ATN Coin"
        elif "ate" in message.lower() or "eat" in message.lower() or "اكل" in message.lower() or "أكل" in message.lower():
...
def setup_processing_system(app: "FastAPI"):
    ...
    async def voice(request: Request, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
        ...
        with open("temp.wav", "rb") as f:
            ...
            gpt_message = await send_to_gpt(response.text, user)
    ...
    async def chat(
        ...
        user: User = db.query(User).filter(User.username == current_user).first()
        response = await send_to_gpt(message, user)
